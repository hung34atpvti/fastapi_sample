...
async def create(body: BlogCreateDTO, db: Session = Depends(get_db),
                 current_user: TokenDataDTO = Depends(get_current_user)):
    request_blog = Blog(title=body.title, body=body.body, user_id=current_user.id)
    new_blog = await BlogRepository.create_one(db, request_blog)
    return new_blog


@router.get('/{id}', status_code=status.HTTP_200_OK, response_model=BlogShowDTO)
async def show(id, db: Session = Depends(get_db), current_user: TokenDataDTO = Depends(get_current_user)):
...
async def destroy(id, db: Session = Depends(get_db), current_user: TokenDataDTO = Depends(get_current_user)):
    return BlogRepository.remove_one(db, id)


@router.get('', response_model=List[BlogShowDTO])
async def show_all(db: Session = Depends(get_db), current_user: TokenDataDTO = Depends(get_current_user)):
